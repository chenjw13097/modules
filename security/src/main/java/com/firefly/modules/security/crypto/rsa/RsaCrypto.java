 ...
 * @date 2019-01-09
 */
public class RsaCrypto implements Crypto<byte[], byte[]> {
    ...
    @Override
    public KeyPair<byte[], byte[]> generateKeyPair() throws CryptoException {
        KeyPairGenerator keyPairGenerator;
        try {
            keyPairGenerator = KeyPairGenerator.getInstance(RSA_ALGORITHM);
        } catch (NoSuchAlgorithmException e) {
            throw new CryptoException(NO_RSA_ALGORITHM_MESSAGE, e);
        }
    ...
    @Override
    public byte[] encryptWithPublicKey(byte[] plainText, byte[] publicKey) throws CryptoException {
        Key key = fromBytes(publicKey, false);
        return processText(plainText, key, Cipher.ENCRYPT_MODE);
    }

    @Override
    public byte[] encryptWithPrivateKey(byte[] plainText, byte[] privateKey) throws CryptoException {
        Key key = fromBytes(privateKey, true);
        return processText(plainText, key, Cipher.ENCRYPT_MODE);
    }

    @Override
    public byte[] decryptWithPublicKey(byte[] cipherText, byte[] publicKey) throws CryptoException {
        Key key = fromBytes(publicKey, false);
        return processText(cipherText, key, Cipher.DECRYPT_MODE);
    }

    @Override
    public byte[] decryptWithPrivateKey(byte[] cipherText, byte[] privateKey) throws CryptoException {
        Key key = fromBytes(privateKey, true);
        return processText(cipherText, key, Cipher.DECRYPT_MODE);
    }

    private Key fromBytes(byte[] bytesKey, boolean isPrivate) throws CryptoException {
        ...
        try {
            ...
            throw new CryptoException(String.format("Fails to get key from %s", HexBin.encode(bytesKey)), e);
        }

        return key;
    }

    private byte[] processText(byte[] text, Key key, int mode) throws CryptoException {
        Cipher cipher;
        try {
            ...
            throw new CryptoException(String.format("Getting cipher of %s goes wrong", RSA_ALGORITHM), e);
        }
        ...
        try {
            ...
            throw new CryptoException("An exception happens when encrypt plain text or decrypt cipher text", e);
        }
    }

    @Override
    public byte[] sign(byte[] message, byte[] privateKey) throws CryptoException {
        Signature signature;
        try {
            signature = Signature.getInstance(SHA256_WITH_RSA_ALGORITHM);
        } catch (NoSuchAlgorithmException e) {
            throw new CryptoException(NO_SHA256_WITH_RSA_ALGORITHM_MESSAGE, e);
        }
        ...
        try {
            ...
            throw new CryptoException("An exception happens when sign message", e);
        }
    }

    @Override
    public boolean verify(byte[] message, byte[] messageDigest, byte[] publicKey) throws CryptoException {
        Signature signature;
        try {
            signature = Signature.getInstance(SHA256_WITH_RSA_ALGORITHM);
        } catch (NoSuchAlgorithmException e) {
        ...
        try {
            ...
            throw new CryptoException("An exception happens when verify message against digest", e);
        }
    }
}
